			...
			try {
				...
				try {
					...
					try {
						...
						console.warn("直接解码失败，使用备用方法:", error);

						// 备用方法：使用MediaRecorder
						const audioContext = getAudioContext();
	...
	const mergeAudioFiles = useCallback(async () => {
		...
		try {
			...
			for (let i = 0; i < audioFiles.length; i++) {
				...
				// 确保buffer存在再使用
				if (!buffer) {
					console.warn(`跳过文件 ${audioFile?.name}，无法获取音频数据`);
					continue;
				}

				// 如果需要重新采样
				if (buffer.sampleRate !== targetSampleRate) {
					buffer = await resampleAudioBuffer(
						audioContext,
						buffer,
						targetSampleRate,
					);
			...
			switch (audioSettings.format) {
				case "mp3":
					audioBlob = await audioBufferToMp3(
						mergedBuffer,
					...
					break;
				case "ogg":
					audioBlob = await audioBufferToOgg(
						mergedBuffer,
		...
		switch (settings.format) {
			...
			case "ogg":
				...
				const compressionRatio =
					settings.quality === "ultra-low"
						? 8
						: settings.quality === "low"
							? 4
							: 2;
				sizeInBytes =
					(settings.sampleRate * 8 * 1 * duration) / (8 * compressionRatio);
				break;
			default:
				sizeInBytes = (settings.sampleRate * 16 * 2 * duration) / 8;
		}
	...
	return (
		<div className="mx-auto max-w-6xl p-6">
			...
			<div className="mb-8 rounded-lg border border-gray-200 bg-white p-6">
				<h3 className="mb-4 font-semibold text-lg">🎛️ 音频设置</h3>
				{/* 快速预设 */}
				<div className="mb-4">
					...
					<div className="flex flex-wrap gap-2">
						<button
							...
							className="rounded-full bg-green-100 px-3 py-1 text-green-700 text-xs hover:bg-green-200"
						>
							🎵 音乐质量
						</button>
						<button
						...
						<button
							...
							className="rounded-full bg-orange-100 px-3 py-1 text-orange-700 text-xs hover:bg-orange-200"
						>
							📱 手机传输
						</button>
						<button
							...
							className="rounded-full bg-red-100 px-3 py-1 text-red-700 text-xs hover:bg-red-200"
						>
							🚀 极限压缩
						</button>
					</div>
				...
				<div className="grid grid-cols-1 gap-4 md:grid-cols-4">
					...
					<div>
						...
						<select
							...
							onChange={(e) =>
								setAudioSettings((prev) => ({
									...prev,
									sampleRate: Number.parseInt(e.target.value),
								}))
							}
							className="w-full rounded-lg border border-gray-300 p-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
						>
					...
					<div>
						...
						<select
							...
							className="w-full rounded-lg border border-gray-300 p-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
						>
				...
				<div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-3">
					<div className="flex items-center justify-between">
						<p className="text-blue-700 text-sm">
							...
							{audioSettings.format === "ogg" &&
								audioSettings.quality === "ultra-low" &&
								"超小文件，语音专用"}
							{audioSettings.format === "ogg" &&
								audioSettings.quality !== "ultra-low" &&
								"压缩文件，节省空间"}
						...
						{audioFiles.length > 0 && (
							<div className="text-right">
								<p className="text-gray-600 text-xs">预估合并后大小</p>
			...
			<div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2">
				{/* 音频文件上传 */}
				<div className="rounded-lg border-2 border-gray-300 border-dashed p-6 text-center transition-colors hover:border-blue-400">
					<input
						ref={fileInputRef}
						type="file"
						accept="audio/*"
					...
					<h3 className="mb-2 font-semibold text-lg">上传音频文件</h3>
					<p className="mb-4 text-gray-500">支持 MP3、WAV、OGG 等格式</p>
					<button
				...
				{/* 视频文件上传 */}
				<div className="rounded-lg border-2 border-gray-300 border-dashed p-6 text-center transition-colors hover:border-purple-400">
					<input
						ref={videoInputRef}
						type="file"
						accept="video/*"
					...
					<h3 className="mb-2 font-semibold text-lg">提取视频音频</h3>
					<p className="mb-4 text-gray-500">从视频文件中提取音频轨道</p>
					<button
							...
							>
									...
									<div>
										...
										{audioFile.duration && (
											<p className="text-gray-500 text-sm">
												时长: {formatDuration(audioFile.duration)}
											</p>
										)}
									</div>
								</div>
								...
								<div className="flex items-center space-x-2">
									<button
										...
										title={
											currentlyPlaying === audioFile.id ? "停止播放" : "播放"
										}
									>
			...
			{mergedAudio && (
				<div className="mb-8">
					<h2 className="mb-4 font-semibold text-xl">合并后的音频</h2>
					<div className="flex items-center justify-between rounded-lg border border-green-200 bg-green-50 p-4">
						<div className="flex items-center space-x-3">
							...
							<div>
								...
								{mergedAudio.duration && (
									<p className="text-gray-500 text-sm">
										总时长: {formatDuration(mergedAudio.duration)}
									</p>
								)}
							</div>
						</div>
						...
						<div className="flex items-center space-x-2">
							<button
								...
								className="rounded p-2 text-blue-500 hover:bg-blue-50"
								title={
									currentlyPlaying === mergedAudio.id ? "停止播放" : "播放"
								}
							>
			...
			{extractedAudio && (
				<div className="mb-8">
					<h2 className="mb-4 font-semibold text-xl">从视频提取的音频</h2>
					<div className="flex items-center justify-between rounded-lg border border-purple-200 bg-purple-50 p-4">
						<div className="flex items-center space-x-3">
							...
							<div>
								...
								{extractedAudio.duration && (
									<p className="text-gray-500 text-sm">
										时长: {formatDuration(extractedAudio.duration)}
									</p>
								)}
							</div>
						</div>
						...
						<div className="flex items-center space-x-2">
							<button
								...
								className="rounded p-2 text-blue-500 hover:bg-blue-50"
								title={
									currentlyPlaying === extractedAudio.id ? "停止播放" : "播放"
								}
							>
			...
			<div className="rounded-lg border border-gray-200 bg-gray-50 p-6">
				<h3 className="mb-3 font-semibold text-lg">💡 使用提示</h3>
				<ul className="space-y-2 text-gray-600 text-sm">
					<li>• 支持上传多个音频文件，按上传顺序进行合并</li>
					<li>• 可以从视频文件中提取音频轨道，支持常见视频格式</li>
					<li>
						• <strong>🚀 极限压缩：</strong>
						最高可减少95%文件大小，特别适合语音录音
					</li>
					<li>
						• <strong>📱 快速预设：</strong>
						一键选择音乐质量、语音标准、手机传输、极限压缩
					</li>
					<li>
						• <strong>📊 实时预估：</strong>设置调整时实时显示预估文件大小
					</li>
					<li>
						• <strong>🎛️ 精细控制：</strong>采样率、比特率、位深度全面可调
					</li>
	...
	for (let channel = 0; channel < buffer.numberOfChannels; channel++) {
		...
		for (let i = 0; i < newLength; i++) {
			...
			if (index + 1 < oldData.length) {
				newData[i] =
					(oldData[index] || 0) * (1 - fraction) +
					(oldData[index + 1] || 0) * fraction;
			} else {
...
async function audioBufferToMp3(
	...
	return new Promise((resolve, reject) => {
		try {
			...
			// 回退到主线程编码
			return audioBufferToMp3Fallback(buffer, bitRate)
				.then(resolve)
				.catch(reject);
		}
	});
...
// MP3编码备用方法（在主线程）
async function audioBufferToMp3Fallback(
	buffer: AudioBuffer,
	bitRate: number,
): Promise<Blob> {
	try {
		...
		for (let i = 0; i < left.length; i++) {
			...
			rightInt16[i] = Math.max(
				-32768,
				Math.min(32767, (right[i] || 0) * 32767),
			);
		}
...
async function audioBufferToOgg(
	buffer: AudioBuffer,
	quality: AudioQuality,
): Promise<Blob> {
	// 根据质量设置调整位深度和采样率
...
// 超级压缩的WAV格式
function audioBufferToWavUltraCompressed(
	buffer: AudioBuffer,
	bitDepth = 8,
	compressionRatio = 4,
): Blob {
	...
	const arrayBuffer = new ArrayBuffer(
		44 + compressedLength * numberOfChannels * bytesPerSample,
	);
	...
	view.setUint32(
		4,
		36 + compressedLength * numberOfChannels * bytesPerSample,
		true,
	);
	writeString(8, "WAVE");
	writeString(12, "fmt ");
	...
	view.setUint32(
		40,
		compressedLength * numberOfChannels * bytesPerSample,
		true,
	);
	...
	for (let i = 0; i < compressedLength; i++) {
		...
		if (sourceIndex < sourceData.length) {
			...
			const intSample = Math.max(
				-maxValue,
				Math.min(maxValue, sample * maxValue),
			);
			...
			if (bitDepth === 16) {
				...
				// 其他位深度
				const reducedSample =
					Math.round(intSample / Math.pow(2, 16 - bitDepth)) *
					Math.pow(2, 16 - bitDepth);
...
// 压缩的WAV格式
function audioBufferToWavCompressed(buffer: AudioBuffer, bitDepth = 16): Blob {
	const length = buffer.length;
